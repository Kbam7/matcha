...
if (isset($_SESSION['logged_on_user']) && $_POST['submit'] === '1') {
    if (isset($_POST['getDashboardUsers']) && $_POST['getDashboardUsers'] === '1') {
        getDashboardUsers();
    } else {
        try {
...
// Return user profiles to display on dashboard
function getDashboardUsers()
{
    ...
    $results = $client->run('MATCH (u:User {username: {uname}})-[:HAS_INTEREST]->(i:Interest)<-[:HAS_INTEREST]-(u2:User) '
                        ...
                        .'RETURN DISTINCT u2', ['uname' => $_SESSION['logged_on_user']['username']]);
