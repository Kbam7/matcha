...
if (isset($_SESSION['logged_on_user']) && $_POST['submit'] === '1') {
    try {
        ...
        // GENDER
        if ($_POST['gender'] && strlen($_POST['gender']) && $_POST['gender'] !== $user['gender']) {
        ...
        // SEXUAL PREFERENCE - LOGIC
            ...
            if ($_POST['sex_pref_m'] && !in_array('men', $curr, true)) {
                // Add it to the array
                array_push($curr, 'men');
                $statusMsg .= '<p class="alert alert-success">Sexual Preference updated.</p>';
            } elseif (!$_POST['sex_pref_m'] && in_array('men', $curr, true)) {
                // remove it from array
                array_splice($curr, array_search('men', $curr), 1);
                $statusMsg .= '<p class="alert alert-success">Sexual Preference updated.</p>';
            }

        if ($_POST['sex_pref_f'] && !in_array('women', $curr, true)) {
            // Add it to the array
                array_push($curr, 'women');
            $statusMsg .= '<p class="alert alert-success">Sexual Preference updated.</p>';
        } elseif (!$_POST['sex_pref_f'] && in_array('women', $curr, true)) {
            // remove it from array
